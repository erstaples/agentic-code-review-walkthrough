...
// Check parsed JSON before it enters the typed model.
...
const entityFields: Record<string, Check> = {
  provenance,
  answers: optional(
...
const manifestFileFields: Record<string, Check> = {
  path: string,
...
const manifestFields: Record<string, Check> = {
...
export function validateEventPayload(value: unknown): void {
  invariant(
    ...
    "unknown_event",
    `unknown event type: ${isRecord(value) ? value.eventType : undefined}`,
  );
  const check = payloadChecks[value.eventType as keyof Payloads];
  invariant(
    check(value.payload),
...
export function readActor(value: unknown): Actor {
  invariant(actor(value), "invalid_actor", "actor requires kind and id");
...
export function readEntityInput(value: unknown): EntityInput {
  invariant(
    entityInput(value),
    "invalid_command",
    "entity fields have invalid types",
  );
  return value as EntityInput;
}
export function readCommand(value: unknown): Command {
  invariant(
  ...
  if (
    ...
    return { type: value.type as AddInput["type"], entity };
  }
  const checks: Record<string, Check> = {
    ...
    MarkPrepared: isRecord,
    ArchiveReviewMap: isRecord,
...
>(method: K, value: unknown): import("./types.js").MapRequests[K] {
  invariant(isRecord(value), "bad_request", "tool arguments must be an object");
  const fields: Record<string, Check> = { workspace: string };
  ...
  return value as import("./types.js").MapRequests[K];
}

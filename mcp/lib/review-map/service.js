...
class ReviewMapService {
  ...
  loadTour(args) {
    ...
    invariant(fresh.freshness === "current", "stale_change", "Refresh the review map before loading changed sources.", fresh);
  ...
  open(args) {
    ...
    if (lineageRelated) return { found: false, freshness: "stale", resolvedChange: { kind: identity.kind, manifestDigest: identity.manifestDigest }, relatedMapId: lineageRelated.id, relatedAggregateRevision: lineageRelated.aggregateRevision, requiredNextAction: "refresh" };
    if (args.createIfMissing === false) return { found: false, freshness: "missing", resolvedChange: { kind: identity.kind, manifestDigest: identity.manifestDigest }, relatedMapId: null, requiredNextAction: "create" };
    const state = this.store.create({ identity, title: args.title, actor: args.actor });
    return { ...projectionOverview(state), freshness: "current", requiredNextAction: "prepare", storage: this.store.inspect(identity.repositoryKey, state.id), relatedMapId: related?.id || null };
  }
  ...
  locate(workspace, mapId) {
    ...
    return { repository, state: this.store.load(repository.repositoryKey, mapId) };
  }
  ...
  apply(args) {
    ...
    for (const command of commands) {
      ...
      const blob = this.store.storeEvidenceBlob(repository.repositoryKey, args.mapId, bytes);
      delete evidence.rawOutput; delete evidence.rawBytesBase64;
      evidence.blobRef = { ...blob, encoding };
      blobsStored++;
    }
    const result = this.store.mutate(repository.repositoryKey, args.mapId, args.expectedRevision, args.actor, (initialState) => {
      if (commands.some((command) => REVIEW_COMMANDS.has(command.type))) {
        const fresh = this.freshness(initialState);
        invariant(fresh.freshness === "current", "stale_change", "the reviewed change no longer matches this review map; refresh before recording review state", fresh);
      }
      ...
      for (const command of commands) {
        if (command.type === "CreateTourPlan") {
          const fresh = this.freshness(state);
          invariant(fresh.freshness === "current", "stale_change", "refresh the review map before authoring a tour for changed sources", fresh);
    ...
    return { mapId: result.state.id, aggregateRevision: result.state.aggregateRevision, emittedEventIds: result.emittedEventIds, changed: projectionOverview(result.state), warnings, findings };
  }
  ...
  delete(args) {
    validateActor(args.actor);
    invariant(args.confirmMapId === args.mapId, "confirmation_required", "confirmMapId must exactly match mapId");
...
module.exports = { ReviewMapService, selectionFor, compareManifests, redactSensitive };

...
export interface ChangeRevision {
  ...
  createdAt: string;
  kind: "committed" | "working-tree";
  labels: { base?: string; head?: string; baseline?: string };
  manifest: Manifest;
...
export interface EntityInput {
  ...
  blobRef?: { digest: string; size: number; encoding: string };
  answers?: { answer: unknown; provenance: Provenance[]; recordedAt: string }[];
  [key: string]: unknown;
}
...
export interface EventPayloads {
  ...
  ThesisSet: { thesis: Thesis };
  RelationshipAdded: { relationship: Relationship };
  TourPlanCreated: { plan: StoredPlan };
  ReviewMapPrepared: Record<string, never>;
  ReviewSessionStarted: { session: ReviewSession };
  StopStarted: { sessionId: string; stopId: string };
  ...
  PauseReviewSession: { sessionId: string; outcome: string | null };
  ResumeReviewSession: { sessionId: string; outcome: string | null };
  CompleteReviewSession: { sessionId: string; outcome: string | null };
  EntityCorrected: {
    collection: Collection;
    entityId: string;
  ...
  ReceiptEmitted: { receiptRef: ReceiptRef };
...
export type Command =
  ...
  | { type: "StartReviewSession"; sessionId?: string; reviewer?: Actor }
  | { type: "StartStop"; sessionId: string; stopId: string }
  | {
...
export interface MapRequests {
  ...
  get: MapLocation & { selector?: Selector };
  apply: Mutation & { commands: unknown[] };
  check: MapLocation;
  refresh: Mutation & { selection: Selection };
  receipt: MapLocation & {
    sessionId: string;
    mode: string;
    actor: Actor;
  ...
  delete: MapLocation & { confirmMapId: string; actor: Actor };
}

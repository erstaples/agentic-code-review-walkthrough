...
import { readActor, readCommand } from "./readers.js";
...
const ENTITY_COLLECTIONS: Record<EntityKind, Collection> = {
...
function validateActor(actor: unknown): asserts actor is Actor {
  readActor(actor);
}

function validateProvenance(provenance: Provenance[], required = true) {
  invariant(
...
function findEntity(state: Aggregate, entityId: string) {
...
function requireEntity(state: Aggregate, entityId: string) {
...
function validateCodeReference(ref: EntityInput, workspace: string) {
  invariant(
...
function eventForCommand(
  ...
  if (
    ...
    return {
      eventType: `${command.type.slice(3)}Added` as EntityEvent,
      payload: made,
    };
  }
  if (command.type === "RecordQuestion" || command.type === "RecordConcern") {
    const noun = command.type.slice(6);
    const alias: AddEntityCommand = {
      ...command,
      type: `Add${noun}` as AddEntityCommand["type"],
    };
    ...
    return { eventType: `${noun}Recorded` as EntityEvent, payload: made };
  }
...
function applyEvent(state: Aggregate | null, event: AppliedEvent): Aggregate {
  if (event.eventType === "ReviewMapCreated")
    ...
    if (eventType === "ThesisSet") state.thesis = p.thesis;
      ...
      invariant(stop, "corrupt_history", `stop not found: ${p.stopId}`);
...
function currentRevision(state: Aggregate) {
  const revision = state.changeRevisions.find(
    (item) => item.id === state.currentChangeRevisionId,
  );
  invariant(revision, "corrupt_history", "current change revision is missing");
  return revision;
}

function replay(events: StoredEvent[]): Aggregate {
  const state = events.reduce<Aggregate | null>(
    (state, event) => applyEvent(state, event),
    null,
  );
  invariant(state, "corrupt_history", "event history is empty");
  return state;
}

function projectionOverview(state: Aggregate) {
  const values = (name: Collection) =>

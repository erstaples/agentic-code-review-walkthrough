...
const { FileChangeRecordStore } = require("./file-store.js");
...
class ChangeRecordService {
  constructor(options = {}) {
    this.store = options.store || new FileChangeRecordStore(options);
  ...
  loadTour(args) {
    const { state } = this.locate(args.workspace, args.recordId);
    const fresh = this.freshness(state);
    invariant(fresh.freshness === "current", "stale_change", "Refresh the record before loading changed sources.", fresh);
  ...
  open(args) {
    ...
    if (lineageRelated) return { found: false, freshness: "stale", resolvedChange: { kind: identity.kind, manifestDigest: identity.manifestDigest }, relatedRecordId: lineageRelated.id, relatedAggregateRevision: lineageRelated.aggregateRevision, requiredNextAction: "refresh" };
    if (args.createIfMissing === false) return { found: false, freshness: "missing", resolvedChange: { kind: identity.kind, manifestDigest: identity.manifestDigest }, relatedRecordId: null, requiredNextAction: "create" };
    const state = this.store.create({ identity, title: args.title, actor: args.actor });
    return { ...projectionOverview(state), freshness: "current", requiredNextAction: "prepare", storage: this.store.inspect(identity.repositoryKey, state.id), relatedRecordId: related?.id || null };
  }
  ...
  locate(workspace, recordId) {
    const repository = repositoryIdentity(workspace);
    return { repository, state: this.store.load(repository.repositoryKey, recordId) };
  }
  ...
  get(args) {
    const { repository, state } = this.locate(args.workspace, args.recordId);
  ...
  apply(args) {
    ...
    const findings = [];
    const { repository } = this.locate(args.workspace, args.recordId);
    let blobsStored = 0;
    for (const command of commands) {
      ...
      const blob = this.store.storeEvidenceBlob(repository.repositoryKey, args.recordId, bytes);
      delete evidence.rawOutput; delete evidence.rawBytesBase64;
      evidence.blobRef = { ...blob, encoding };
      blobsStored++;
    }
    const result = this.store.mutate(repository.repositoryKey, args.recordId, args.expectedRevision, args.actor, (initialState) => {
      if (commands.some((command) => REVIEW_COMMANDS.has(command.type))) {
        const fresh = this.freshness(initialState);
        invariant(fresh.freshness === "current", "stale_change", "the reviewed change no longer matches this record; refresh before recording review state", fresh);
      }
      ...
      for (const command of commands) {
        if (command.type === "CreateTourPlan") {
          const fresh = this.freshness(state);
          invariant(fresh.freshness === "current", "stale_change", "refresh the record before authoring a tour for changed sources", fresh);
    ...
    return { recordId: result.state.id, aggregateRevision: result.state.aggregateRevision, emittedEventIds: result.emittedEventIds, changed: projectionOverview(result.state), warnings, findings };
  }
  ...
  check(args) {
    ...
    return { ok: freshness.freshness === "current" && !findings.some((f) => f.severity === "error"), recordId: state.id, aggregateRevision: state.aggregateRevision, schemaValid: state.schemaVersion === 1, eventChainValid: true, ...freshness, findings };
  }
  ...
  delete(args) {
    validateActor(args.actor);
    invariant(args.confirmRecordId === args.recordId, "confirmation_required", "confirmRecordId must exactly match recordId");
    const repository = repositoryIdentity(args.workspace);
    this.store.delete(repository.repositoryKey, args.recordId);
    return { deleted: true, recordId: args.recordId, recoverable: false };
  }
}
...
module.exports = { ChangeRecordService, selectionFor, compareManifests, redactSensitive };

...
function buildReceipt(state, sessionId, options = {}) {
  ...
  const change = state.changeRevisions.find((item) => item.id === session.changeRevisionId);
  invariant(change, "change_revision_not_found", `change revision not found: ${session.changeRevisionId}`);

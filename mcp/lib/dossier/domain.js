...
function newAggregate({ dossierId, title, repository, changeRevision, actor, occurredAt }) {
  return {
    schemaVersion: SCHEMA_VERSION, producerVersion: PRODUCER_VERSION, id: dossierId, title,
...
function eventForCommand(state, command, actor, occurredAt = now()) {
  ...
  validateActor(actor);
  if (state.phase === "archived" && command.type !== "EmitReceipt") throw new DossierError("dossier_archived", "the dossier is archived and read-only");
  ...
  if (command.type === "MarkPrepared") {
    ...
    return { eventType: "DossierPrepared", payload: {} };
  }
  if (command.type === "StartReviewSession") {
    invariant(state.phase === "prepared", "not_prepared", "prepare the dossier before starting review");
  ...
  if (command.type === "ArchiveDossier") return { eventType: "DossierArchived", payload: {} };
  throw new DossierError("unknown_command", `unknown dossier command: ${command.type}`);
}

function applyEvent(state, event) {
  if (event.eventType === "DossierCreated") state = newAggregate(event.payload);
  else {
    invariant(state, "corrupt_history", "event history does not begin with DossierCreated");
    ...
    else if (event.eventType === "DossierPrepared") state.phase = "prepared";
    ...
    else if (event.eventType === "DossierArchived") { state.phase = "archived"; state.archivedAt = event.occurredAt; }
    else throw new DossierError("unknown_event", `unknown event type: ${event.eventType}`);
  }
...
function projectionOverview(state) {
  ...
  return {
    dossierId: state.id, title: state.title, phase: state.phase, aggregateRevision: state.aggregateRevision,

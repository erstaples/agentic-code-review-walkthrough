...
function fixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "tour-dossier-test-"));
  ...
  return { root, workspace, stateRoot, git, base, head, service: new DossierService({ root: stateRoot }) };
}

function open(f) {
  return f.service.open({ workspace: f.workspace, selection: { kind: "committed", base: f.base, head: f.head }, actor, title: "Test change" });
}

function prepare(f, opened) {
  let result = f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: opened.aggregateRevision, actor, commands: [
  ...
  const tour = f.service.get({ workspace: f.workspace, dossierId: opened.dossierId, selector: { kind: "tour" } });
  return { result, claim, session: tour.sessions[0], stop: tour.plan.stops[0] };
}

test("a committed dossier survives service restart with a verified event chain", () => {
  const f = fixture(); const opened = open(f); const prepared = prepare(f, opened);
  const restarted = new DossierService({ root: f.stateRoot });
  const overview = restarted.get({ workspace: f.workspace, dossierId: opened.dossierId, selector: { kind: "overview" } });
  ...
  assert.strictEqual(restarted.check({ workspace: f.workspace, dossierId: opened.dossierId }).eventChainValid, true);
});

test("optimistic aggregate revisions reject stale writers", () => {
  const f = fixture(); const opened = open(f);
  f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: 1, actor, commands: [{ type: "SetThesis", thesis: { summary: "A", provenance } }] });
  assert.throws(() => f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: 1, actor, commands: [{ type: "SetThesis", thesis: { summary: "B", provenance } }] }), (error) => error.code === "revision_conflict" && error.details.currentRevision === 2);
});
...
test("working-tree byte changes are detected before review state is written", () => {
  ...
  assert.strictEqual(f.service.check({ workspace: f.workspace, dossierId: opened.dossierId }).freshness, "stale");
  assert.throws(() => f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: prepared.result.aggregateRevision, actor, commands: [{ type: "StartStop", sessionId: prepared.session.id, stopId: prepared.stop.id }] }), (error) => error.code === "stale_change");
});
...
test("refresh preserves history while invalidating review and evidence conservatively", () => {
  ...
  let result = f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: prepared.result.aggregateRevision, actor, commands: [
  ...
  fs.writeFileSync(path.join(f.workspace, "feature.txt"), "dirty two\n");
  const refreshed = f.service.refresh({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: result.aggregateRevision, selection: { kind: "working-tree" }, actor });
...
test("event corruption is detected instead of silently replayed", () => {
  const f = fixture(); const opened = open(f);
  const store = new FileDossierStore({ root: f.stateRoot });
  const repositoryKey = repositoryIdentity(f.workspace).repositoryKey;
  const event = path.join(store.dossierDir(repositoryKey, opened.dossierId), "events", "000000000001.json");
  const body = JSON.parse(fs.readFileSync(event, "utf8")); body.payload.title = "tampered"; fs.writeFileSync(event, JSON.stringify(body));
  assert.throws(() => store.load(repositoryKey, opened.dossierId), (error) => error.code === "event_hash_invalid");
});
...
test("staged-only identity ignores out-of-scope unstaged bytes", () => {
  ...
  const staged = resolveChange(f.workspace, { kind: "working-tree", includeStaged: true, includeUnstaged: false, includeUntracked: false });
  fs.writeFileSync(path.join(f.workspace, "feature.txt"), "unstaged after index\n");
  const stagedAgain = resolveChange(f.workspace, { kind: "working-tree", includeStaged: true, includeUnstaged: false, includeUntracked: false });
...
test("raw evidence is stored by digest and omitted from event JSON", () => {
  const f = fixture(); const opened = open(f);
  const result = f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: 1, actor, commands: [{ type: "AddEvidence", evidence: { observation: "A bounded test output.", rawOutput: "PASS secret-free output", provenance } }] });
  assert.match(result.warnings.join(" "), /evidence blob/);
  const evidence = f.service.get({ workspace: f.workspace, dossierId: opened.dossierId, selector: { kind: "entities", type: "evidence" } }).entities[0];
  assert.match(evidence.blobRef.digest, /^sha256:/); assert.strictEqual(evidence.rawOutput, undefined);
  const repositoryKey = repositoryIdentity(f.workspace).repositoryKey;
  const eventPath = path.join(new FileDossierStore({ root: f.stateRoot }).dossierDir(repositoryKey, opened.dossierId), "events", "000000000002.json");
...
test("code references are resolved to exact revisions and content digests", () => {
  const f = fixture(); const opened = open(f);
  f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: 1, actor, commands: [{ type: "AddCodeReference", codeReference: { path: "feature.txt", side: "head", startLine: 1, endLine: 1, provenance } }] });
  const reference = f.service.get({ workspace: f.workspace, dossierId: opened.dossierId, selector: { kind: "entities", type: "codeReferences" } }).entities[0];
...
test("material review questions use the RecordQuestion command vocabulary", () => {
  const f = fixture(); const opened = open(f); const prepared = prepare(f, opened);
  f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: prepared.result.aggregateRevision, actor, commands: [{ type: "RecordQuestion", question: { question: "Does this preserve compatibility?", relatedEntityIds: [prepared.claim.id], provenance: [{ kind: "reviewer-stated", source: { type: "review-session", id: prepared.session.id } }] } }] });
  const openItems = f.service.get({ workspace: f.workspace, dossierId: opened.dossierId, selector: { kind: "open-items" } });
...
test("secret-shaped command content is redacted before persistence", () => {
  const sample = { token: "abc", note: "Authorization: Bearer abc.def.ghi", key: "AKIA1234567890ABCDEF" };
  const result = redactSensitive(sample);
  assert.strictEqual(result.redacted, true);
  assert.strictEqual(result.value.token, "[REDACTED]");
  assert.doesNotMatch(JSON.stringify(result.value), /abc\.def|AKIA123/);
});

test("dossier deletion requires exact confirmation and is narrowly scoped", () => {
  const f = fixture(); const opened = open(f);
  assert.throws(() => f.service.delete({ workspace: f.workspace, dossierId: opened.dossierId, confirmDossierId: "dos_00000000-0000-0000-0000-000000000000", actor }), (error) => error.code === "confirmation_required");
  assert.deepStrictEqual(f.service.delete({ workspace: f.workspace, dossierId: opened.dossierId, confirmDossierId: opened.dossierId, actor }), { deleted: true, dossierId: opened.dossierId, recoverable: false });
  assert.throws(() => f.service.get({ workspace: f.workspace, dossierId: opened.dossierId, selector: { kind: "overview" } }), (error) => error.code === "dossier_not_found");
});
...
test("forceNew separates distinct tasks that begin from the same working tree", () => {
  ...
  assert.strictEqual(reused.dossierId, first.dossierId);
  const second = f.service.open({ workspace: f.workspace, selection, actor, title: "Second task", forceNew: true });
  assert.notStrictEqual(second.dossierId, first.dossierId);
});

...
test("state directories use the product namespace on every platform", () => {
  assert.equal(stateRoot({ KANKO_STATE_DIR: "custom-state" }), path.resolve("custom-state"));
  assert.equal(stateRoot({}, "darwin"), path.join(os.homedir(), "Library", "Application Support", "kanko"));
  assert.equal(stateRoot({ LOCALAPPDATA: "/local" }, "win32"), path.join("/local", "kanko"));
  assert.equal(stateRoot({ XDG_STATE_HOME: "/state" }, "linux"), path.join("/state", "kanko"));
...
test("the stdio server opens, updates, and reloads a review map through its advertised names", { timeout: 15000 }, async t => {
  ...
  t.after(async () => { lines.close(); child.kill(); await exited; fs.rmSync(fixture.root, { recursive: true, force: true }); });
  let id = 0;
  ...
  async function call(name, args) {
    const result = await rpc("tools/call", { name, arguments: { workspace: fixture.workspace, ...args } });

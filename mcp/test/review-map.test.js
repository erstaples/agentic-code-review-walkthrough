...
const {
  ReviewMapService,
  redactSensitive,
} = require("../lib/review-map/service.js");
...
test("optimistic aggregate revisions reject stale writers", () => {
  ...
  assert.throws(
    ...
    (error) =>
      error.code === "revision_conflict" && error.details.currentRevision === 2,
  );
});
...
test("raw evidence is stored by digest and omitted from event JSON", () => {
  ...
  assert.match(evidence.blobRef.digest, /^sha256:/);

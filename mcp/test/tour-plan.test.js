...
) => f.service.open({ workspace: f.workspace, selection, actor });
const apply = (
  f,
  opened,
  commands,
...
test("unversioned and incomplete plans cannot bypass authoring validation", (t) => {
  ...
  delete unversioned.presentationVersion;
  const metadataOnly = {
    type: "CreateTourPlan",
  ...
  for (const cmd of [
    ...
    assert.throws(
      () => apply(f, opened, [cmd]),
      (e) => e.code === "invalid_tour_plan" && e.details.findings.length > 0,
    );
  }
  assert.equal(getTour(f, opened), null);
  assert.equal(
...
test("checking a stored plan always validates its version and required fields", (t) => {
  ...
  delete stored.presentationVersion;
  delete stored.stops[0].anchors;
...
test("source validation rejects wrong revisions, EOF ranges, and raw catalog paths", (t) => {
  ...
  for (const [data, code] of [
    ...
    assert.throws(
      ...
      (e) => e.details.findings.some((finding) => finding.code === code),
    );
  const cmd = command([anchor(f)]);
  cmd.stops[0].beats[0].narration = "Inspect Makefile.";
  assert.throws(
    () => apply(f, opened, [cmd]),
    (e) => e.details.findings.some((finding) => finding.code === "raw_path"),
  );
});

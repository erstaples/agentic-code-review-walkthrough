...
function fixture(t, serviceOptions = {}) {
  const root = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), "relay-tour-contract-")));
  ...
  const stateRoot = path.join(root, "state"); const service = new DossierService({ root: stateRoot, ...serviceOptions });
...
const apply = (f, opened, commands, expectedRevision = opened.aggregateRevision) => f.service.apply({ workspace: f.workspace, dossierId: opened.dossierId, expectedRevision, actor, commands });
const getTour = (f, opened) => f.service.get({ workspace: f.workspace, dossierId: opened.dossierId, selector: { kind: "tour" } }).plan;
...
test("invalid tour rejects an entire command batch and does not navigate the editor", async (t) => {
  ...
  const dispatcher = createDispatcher({ tools: TOOLS, callTool: createCallTool({ dossierService: f.service, resolveLock: () => { editorRequests++; throw new Error("Unexpected editor access"); } }) });
  const result = await dispatcher.handle({ jsonrpc: "2.0", id: 1, method: "tools/call", params: { name: "dossier_apply", arguments: {
    workspace: f.workspace, dossierId: opened.dossierId, expectedRevision: opened.aggregateRevision, actor,
  ...
  const state = f.service.get({ workspace: f.workspace, dossierId: opened.dossierId, selector: { kind: "overview" } });
...
test("working-tree tours reject drift before storage and report stale hashes on check", (t) => {
  ...
  const check = f.service.check({ workspace: f.workspace, dossierId: opened.dossierId });

...
test("marketplace declares the kanko-tour plugin from the repo root", () => {
  const m = readJson(".claude-plugin/marketplace.json");
  assert.strictEqual(m.name, "kanko");
  const plugin = m.plugins.find((p) => p.name === "kanko-tour");
  assert.ok(plugin, "kanko-tour plugin missing from marketplace");
  assert.strictEqual(plugin.source, "./");
});

test("plugin.json name matches the marketplace entry", () => {
  assert.strictEqual(readJson(".claude-plugin/plugin.json").name, "kanko-tour");
});
...
test("Codex uses a portable manifest and MCP configuration", () => {
  ...
  assert.strictEqual(plugin.name, "kanko-tour");

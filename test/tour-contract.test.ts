import { string } from "./assertions.js";
import type {
  FocusSpan,
  LineRange,
  Beat,
...
interface TestAnchor {
  ...
  view: string;
  change: string;
  rev: { base: string; head: string };
  context: LineRange;
  contentHash: ContentHash;
  focus?: (Omit<FocusSpan, "side"> & { side: string })[];
  side?: string;
  claimRefs?: string[];
}
interface TestPlan {
...
function anchor(n: number, range = { startLine: n, endLine: n }): TestAnchor {
  return {
    n,
    role: "context",
    ...
    context: range,
    contentHash: hashText(string(rangeText(text, range))),
  };
}
function plan(count = 1): TestPlan {
  return {
...
test("tour plans require the current version explicitly", () => {
  for (const presentationVersion of [undefined, null, 1, 3, "2"]) {
    const input = plan();
    Object.assign(input, { presentationVersion });
    const result = validateTourPlan(input, options);
...
const invalid: [string, (p: ReturnType<typeof plan>) => unknown][] = [
  ...
  [
    "invalid_active",
    (p) => Object.assign(p.stops[0].beats[0], { active: null }),
  ],
  [
    "raw_path",
  ...
  ["invalid_anchor", (p) => Object.assign(p.stops[0].anchors, { 0: null })],
];
...
test("anchor limits are configurable through 99 and active overflow is advisory", () => {
  ...
  for (const hardLimit of [0, 100, 1.5, "24", NaN])
    assert.throws(
      () =>
        Reflect.apply(validateTourPlan, undefined, [
          plan(),
          { ...options, hardLimit },
        ]),
      RangeError,
...
test("normalization merges transitive overlaps and rewrites tokens simultaneously", () => {
  ...
  assert.equal(
    ...
    hashText(string(rangeText(text, stop.anchors[0].context))),
  );

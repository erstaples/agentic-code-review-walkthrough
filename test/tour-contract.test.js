...
function anchor(n, range = { startLine: n, endLine: n }) {
  return { n, role: "context", label: "Source context", path: "src/feature.js", view: "head", change: "unchanged", rev: { ...options.revisions }, context: range, contentHash: hashText(rangeText(text, range)) };
}
function plan(count = 1) {
  return { presentationVersion: 2, stops: [{ id: "s1", title: "Behavior", risk: "low", anchors: Array.from({ length: count }, (_, n) => anchor(n + 1)), beats: [{ id: "b1", narration: "Inspect {{a:1}}.", active: [1] }] }] };
}

test("valid v2 tours round-trip without mutating input and retain priority and sides", () => {
  const input = plan(3);
  input.stops[0].beats[0].active = [3, 1, 2];
  input.stops[0].anchors[0].focus = [{ side: "base", range: { startLine: 2, endLine: 3 }, contentHash: hashText("line 2\nline 3") }];
...
const invalid = [
  ...
  ["invalid_label", (p) => p.stops[0].anchors[0].label = "This label has far too many words"],
  ...
  ["invalid_range", (p) => p.stops[0].anchors[0].context = { startLine: 2, endLine: 1 }],
  ["range_out_of_bounds", (p) => p.stops[0].anchors[0].context.endLine = 121],
  ["invalid_focus", (p) => p.stops[0].anchors[0].focus = [{ side: "working", range: { startLine: 1, endLine: 1 } }]],
  ["range_out_of_bounds", (p) => p.stops[0].anchors[0].focus = [{ side: "base", range: { startLine: 121, endLine: 121 } }]],
  ["content_mismatch", (p) => p.stops[0].anchors[0].contentHash = hashText("wrong")],
  ["content_mismatch", (p) => p.stops[0].anchors[0].focus = [{ side: "base", range: { startLine: 1, endLine: 1 }, contentHash: hashText("wrong") }]],
  ["change_mismatch", (p) => p.stops[0].anchors[0].change = "added"],
  ["invalid_beat_id", (p) => p.stops[0].beats.push(structuredClone(p.stops[0].beats[0]))],
  ["missing_anchor", (p) => p.stops[0].beats[0].narration = "Inspect {{a:2}}."],
  ["invalid_anchor_token", (p) => p.stops[0].beats[0].narration = "Inspect {{a:01}}."],
  ["invalid_anchor_token", (p) => p.stops[0].beats[0].narration = "Inspect {{a:1}."],
  ...
  ["raw_path", (p) => p.stops[0].beats[0].narration = "Inspect src/feature.js:1."],
  ["raw_path", (p) => p.stops[0].beats[0].narration = "Inspect `missing.js`."],
  ["raw_path", (p) => p.stops[0].beats[0].narration = "Inspect src/missing.js."],
  ["invalid_beats", (p) => p.stops[0].beats = []],
  ["invalid_anchor", (p) => p.stops[0].anchors[0] = null],
];
for (const [code, mutate] of invalid) test(`rejects ${code}: ${mutate}`, () => {
  const input = plan(); mutate(input);
  const result = validateTourPlan(input, options);
  assert.equal(result.ok, false);
  assert.equal(result.plan, null);
  assert.ok(result.findings.some((f) => f.code === code && f.severity === "error" && f.location.startsWith("stops[")), JSON.stringify(result.findings));
});
...
test("anchor limits are configurable through 99 and active overflow is advisory", () => {
  ...
  assert.ok(validateTourPlan(plan(8), options).findings.some((f) => f.code === "anchor_budget"));
  assert.equal(validateTourPlan(plan(24), options).ok, true);
  assert.equal(validateTourPlan(plan(25), options).ok, false);
  assert.equal(validateTourPlan(plan(99), { ...options, hardLimit: 99 }).ok, true);
  assert.equal(validateTourPlan(plan(100), { ...options, hardLimit: 99 }).ok, false);
  for (const hardLimit of [0, 100, 1.5, "24", NaN]) assert.throws(() => validateTourPlan(plan(), { ...options, hardLimit }), RangeError);
  const input = plan(4); input.stops[0].beats[0].active = [4, 2, 1, 3];
  const result = validateTourPlan(input, options);
  assert.equal(result.ok, true);
...
test("normalization merges transitive overlaps and rewrites tokens simultaneously", () => {
  const input = plan(4);
  input.stops[0].anchors = [anchor(1, { startLine: 1, endLine: 2 }), anchor(2, { startLine: 5, endLine: 6 }), anchor(3, { startLine: 2, endLine: 5 }), anchor(4, { startLine: 8, endLine: 8 })];
  input.stops[0].beats[0] = { id: "b", narration: "{{a:4}}, {{a:2}}, {{a:3}}, {{a:1}}", active: [4, 2, 3, 1] };
  const result = validateTourPlan(input, options);
  assert.equal(result.ok, true);
  ...
  assert.equal(stop.anchors[0].contentHash, hashText(rangeText(text, stop.anchors[0].context)));
...
test("overlap cannot mask bad hashes or erase incompatible roles", () => {
  const input = plan(2); input.stops[0].anchors[1] = anchor(2, { startLine: 1, endLine: 2 });
  input.stops[0].anchors[1].role = "evidence";
  assert.ok(validateTourPlan(input, options).findings.some((f) => f.code === "overlap_conflict"));
...
test("observed claims require claim-specific evidence anchors across the tour", () => {
  const input = plan();
  const claims = [{ id: "clm_a", truthStatus: "observed" }, { id: "clm_b", truthStatus: "observed" }];
  ...
  assert.deepEqual(result.findings.filter((f) => f.code === "observed_without_evidence").map((f) => f.claimId), ["clm_a"]);
...
test("source failures are actionable and catalog paths include bare filenames", () => {
  assert.ok(validateTourPlan(plan()).findings.some((f) => f.code === "source_reader_required"));
  assert.ok(validateTourPlan(plan(), { readSource: () => { throw new Error("Missing commit"); } }).findings.some((f) => f.code === "source_unavailable" && /Missing commit/.test(f.message)));
  const input = plan(); input.stops[0].beats[0].narration = "Inspect Makefile.";
  assert.ok(validateTourPlan(input, { ...options, repositoryPaths: ["Makefile"] }).findings.some((f) => f.code === "raw_path"));
  input.stops[0].beats[0].narration = "Use Node.js with https://example.com/docs/file.html and {{a:1}}.";

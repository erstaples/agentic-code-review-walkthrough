...
[`kanko-tour: product spec`](2026-09-21-kanko-tour-product-spec.md) for
review outcomes, tour state, evidence, provenance, and success metrics.

## Problem

The `kanko-tour` skill narrates a diff stop by stop, citing code as
...
```mermaid
flowchart LR
    C["Claude Code<br/>kanko-tour skill"]
    M["kanko<br/>MCP server (stdio)"]
...
```
codex mcp add kanko -- node "$PLUGIN_ROOT/mcp/server.js"
```
...
kanko` output and re-register if found.
...
file at `skills/kanko-tour/SKILL.md`.
...
`SKILL.md` moves to `skills/kanko-tour/SKILL.md`, which also resolves the
...
- **Step 1 — preflight.** Call `kanko_tour_status`. On success, run the driven tour. On failure, state which capabilities are unavailable, give the one-line install path, and continue as a text-and-links tour. The tour is never blocked on the extension.
...
- **Step 5** ends with `kanko_tour_clear`.
...
- Remote/WSL split installs. The extension must run on the same side as the workspace. `install.sh` installs into the active remote and `kanko_tour_status` reports the resolved workspace so a mismatch is visible immediately.

# kanko-tour Editor Bridge Implementation Plan
...
**Spec:** [`docs/superpowers/specs/2026-09-21-kanko-tour-editor-bridge-design.md`](../specs/2026-09-21-kanko-tour-editor-bridge-design.md)
Product scope deliberately excluded from this plan: [`docs/superpowers/specs/2026-09-21-kanko-tour-product-spec.md`](../specs/2026-09-21-kanko-tour-product-spec.md)
...
- Create: `skills/kanko-tour/SKILL.md` (moved from `SKILL.md`)
- Create: `LICENSE`
- Delete: `SKILL.md`

**Interfaces:**
- Consumes: nothing
- Produces: plugin name `kanko`, marketplace name `kanko`, skill directory `skills/kanko-tour/`
...
```bash
mkdir -p skills/kanko-tour
git mv SKILL.md skills/kanko-tour/SKILL.md 2>/dev/null || mv SKILL.md skills/kanko-tour/SKILL.md
```

Then edit `skills/kanko-tour/SKILL.md`. Replace the frontmatter with:
...
```bash
git add .claude-plugin/marketplace.json .claude-plugin/plugin.json skills/kanko-tour/SKILL.md LICENSE test/plugin-manifest.test.js
git rm --cached SKILL.md 2>/dev/null || true
git commit -m "feat: package kanko-tour as an installable plugin

Moves the skill to skills/kanko-tour/SKILL.md so its name matches its
...
- Produces: MCP tools `kanko_tour_status`, `tour_stop`, `tour_focus`, `kanko_tour_clear`
...
test("the declared tools match the documented surface", () => {
  assert.deepStrictEqual(TOOLS.map((t) => t.name).sort(), ["kanko_tour_clear", "tour_focus", "kanko_tour_status", "tour_stop"]);
...
test("kanko_tour_status GETs /status with the lock's bearer token", async () => {
  ...
  assert.deepStrictEqual(await callTool("kanko_tour_status", {}), { ok: true, extensionVersion: "0.1.0" });
...
test("a discovery failure surfaces its guidance verbatim", async () => {
  ...
  await assert.rejects(() => callTool("kanko_tour_status", {}), /no tour bridge is listening/);
});
...
test("every phase-1 tool consumes its contract response fixtures", async () => {
  ...
  for (const name of ["kanko_tour_status", "tour_stop", "tour_focus", "kanko_tour_clear"]) {
...
**Files:**
- Create: `install.sh`
- Modify: `skills/kanko-tour/SKILL.md`

**Interfaces:**
- Consumes: tool names `kanko_tour_status`, `tour_stop`, `tour_focus`, `kanko_tour_clear` (Task 0 `ROUTES`)
...
if command -v codex >/dev/null; then
  ...
  if codex mcp get kanko 2>/dev/null | grep -q 'CLAUDE_PLUGIN_ROOT'; then
    echo "  Codex did not expand \${CLAUDE_PLUGIN_ROOT}; re-registering with an absolute path"
    codex mcp remove kanko >/dev/null 2>&1 || true
    codex mcp add kanko -- node "$root/mcp/server.js"
  fi
fi
...
Edit `skills/kanko-tour/SKILL.md`. Insert after the "Establish the diff range"
step:

```markdown
### 1b. Preflight the editor bridge

Call `kanko_tour_status`. On success, run a **driven tour**: the editor opens and
...
```bash
git add install.sh skills/kanko-tour/SKILL.md
...
In `skills/kanko-tour/SKILL.md`, replace the "Establish the diff range" body with:
...
```bash
git add skills/kanko-tour/SKILL.md
...
In `skills/kanko-tour/SKILL.md`, under the constraint about answering
interruptions, add:
...
**Files:**
- Modify: `skills/kanko-tour/SKILL.md`
- Create: `skills/kanko-tour/references/ledger.md`
...
In `skills/kanko-tour/SKILL.md`, add a new step before "Close out":
...
```bash
git add skills/kanko-tour/SKILL.md skills/kanko-tour/references/ledger.md
...
Companion extension for the [kanko-tour](https://github.com/getkanko/kanko)

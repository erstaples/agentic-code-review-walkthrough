...
Both revision documents and companions use the read-only `kanko-rev` provider.
...
| --- | --- |
| `kanko.presentation.activate` | `{ anchor, narration?, title?, claimIds? }` |
| `kanko.presentation.detour` | `{ anchor, title }`; temporarily replaces the active beat |
| `kanko.presentation.return` | Restores the saved beat, checking its hashes again |
| `kanko.presentation.follow` | Enables focus dimming and labels |
| `kanko.presentation.pause` | Retains rails, boxes, and labels without dimming |
| `kanko.presentation.status` | Returns `{ state, stale, anchor }` for the panel's drift note |

Claim hover links dispatch `kanko.presentation.openClaim` with a claim ID.

# Relay v2: editor capability verification
...
`Relay: Multi-File Stops and Editor Layout` PDF, especially pages 10-13 and
...
| Do revision documents get language symbols? | Both `file:` and `relay-rev:` TypeScript documents returned `relayProbe`. Go returned `Evidence` for `file:` and no symbols for `relay-rev:` after repeated requests. Real-file positive controls succeeded for both languages. | Prefer a real file only when its content matches the reviewed revision, including unsaved editor contents. Keep revision documents for immutable review. Do not promise Go symbol breadcrumbs on them. TypeScript symbol availability does not establish full project-wide IntelliSense. |
...
```sh
node editor-extension/test/spikes/relay-layout/run.js
```
...
For Go checks, set `RELAY_SPIKE_GO_EXTENSION` to an installed `golang.go`
...
Set `RELAY_SPIKE_OUTPUT` to an empty directory for repeatable artifact collection.
Set `RELAY_SPIKE_MANUAL=1` to leave the test window open after the API checks.
In that window:

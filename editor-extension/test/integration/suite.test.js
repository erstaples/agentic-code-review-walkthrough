...
module.exports = function register({ test, before }) {
  ...
  const service = new ChangeRecordService({ root: fixture.stateRoot });
  let lock, call;
  const results = [];
  ...
  const load = () => api('kanko_tour_load', { recordId: fixture.recordId });
  ...
  before(async () => {
    ...
    assert.ok(lock); call = createCallTool({ recordService: service, resolveLock: () => lock });
  });
  ...
  test('the extension rejects invalid load findings before changing the current tour or tabs', async () => {
    ...
    const bad = service.loadTour({ workspace: fixture.workspace, recordId: fixture.recordId }); bad.plan.stops[0].beats[0].active = [99];
  ...
  test('reusing a real file retains the pinned base source for seam peek', async () => {
    ...
    const payload = service.loadTour({workspace:fixture.workspace,recordId:fixture.recordId});
  ...
  test('clear ends presentation without recording review acceptance, then reload begins at the first beat', async () => {
    ...
    const state = service.locate(fixture.workspace, fixture.recordId).state;
  ...
  if (process.env.KANKO_TOUR_MANUAL) test('manual screenshot acceptance session', async () => {
    fs.writeFileSync(path.join(output, 'ready.json'), JSON.stringify({ workspace: fixture.workspace, stateRoot: fixture.stateRoot, recordId: fixture.recordId }));
    ...
    while (Date.now() < deadline) {
      if (fs.existsSync(file)) {
        ...
        else if (command.action === 'invalid') { const bad = service.loadTour({ workspace: fixture.workspace, recordId: fixture.recordId }); bad.plan.stops[0].beats[0].active = [99]; result = await http('/tour/load', bad); }

...
const { mapRange, seamLineFor, removedBaseLines0 } =
  require("../compiled.js")("src/host/hunks.js");
const {
  normalizeAnchor,
  checkAnchor,
  hashText,
...
function createPresentation(
  ...
  const originalIsVisible = (uri) => {
    ...
    return (
      diff.get("renderSideBySide", true) &&
      !diff.get("useInlineViewWhenSpaceIsLimited", true) &&
      vscode.window.visibleTextEditors.some(
        (e) => uriKey(e.document.uri) === uriKey(uri),
      )
    );
  };
  ...
  const tabs = () =>
    vscode.window.tabGroups.all.flatMap((g) =>
      g.tabs.map((tab) => ({ tab, group: g.viewColumn })),
    );
    ...
    "kanko-rev",
    {
      provideTextDocumentContent(uri) {
        ...
        if (
          ...
          path.isAbsolute(rel) ||
          !/^[0-9a-f]{40,64}$/.test(q.ref)
        )
          throw new Error("invalid revision URI");
  ...
  function revisionUri(
    ...
    const root = editor.workspaceRoot();
    return vscode.Uri.file(path.join(root, source)).with({
      scheme: "kanko-rev",
  ...
  async function closeCompanions(reason, keep) {
    ...
    for (const [key, owned] of companions) {
      const entry = tabs().find(
        ({ tab }) => tab.input?.uri?.toString() === key,
      );
      if (!entry) {
      ...
      if (!entry.tab.isPreview || entry.group !== owned.group)
        owned.detached = true;
      if (
        owned.detached ||
        key === keep ||
        policy === "never" ||
        (reason === "beat" && policy === "onTourEnd")
      )
        continue;
  ...
  async function waitForBase(uri) {
    ...
    await new Promise((resolve) => {
      ...
      const sub = vscode.window.onDidChangeVisibleTextEditors(() => {
        if (find()) finish();
      });
  ...
  async function activate(anchor, stop, label, options = {}) {
    navigating++;
    try {
      return await activateBeat(anchor, stop, label, options);
    } finally {
      navigating--;
    }
  }
  ...
  async function activateBeat(anchor, stop, label, options) {
    ...
    if (
      ...
      throw new Error("anchor path must stay inside the workspace");
    let source = a.path,
      hunks = [],
      changeStatus;
    if (stop?.base && stop?.head) {
      ...
      if (!data) {
        const changes =
          stop.head.sha === "WORKTREE"
            ? []
            : await changedFiles(root, stop.base.sha, stop.head.sha);
        const change = changes.find((c) => c.targetPath === a.path);
        source = change?.sourcePath || a.path;
        data = {
          source,
          hunks: await diffHunks(root, stop.base.sha, stop.head.sha, [
            ...new Set([source, a.path]),
          ]),
      ...
      if (
        stop.head.sha === "WORKTREE" &&
        !(await git.hasBlob(root, stop.base.sha, a.path))
      )
        changeStatus = "A";
    }
    ...
    const readText = async (side) => {
      if (
        ...
        (side === "head" &&
          (stop?.head?.sha === "WORKTREE" ||
            ["worktree", "working"].includes(a.side)))
      ) {
        return (
          await vscode.workspace.openTextDocument(
            vscode.Uri.file(editor.absolute(a.path)),
          )
        ).getText();
      }
      return blobText(
        root,
        stop?.[side]?.sha || a.rev,
        side === "base" ? source : a.path,
      );
    };
    const checked = await checkAnchor(a, readText);
    if (token !== generation) return;
    // Preserve the first activation's independent span hashes on Return.
    a.focus = a.focus.map((f, i) => ({
      ...f,
      contentHash: f.contentHash || checked.focusHashes[i],
      kind:
        f.kind ||
        (hunks.some(
          (h) =>
            h[`${f.side}Len`] > 0 &&
            f.range.startLine < h[`${f.side}Start`] + h[`${f.side}Len`] &&
            f.range.endLine >= h[`${f.side}Start`],
        )
          ? f.side === "base"
            ? "removed"
            : "added"
    ...
    active = {
      ...
      hunks,
      readText,
      hashes: checked.focusHashes,
      stale: checked.stale,
      token,
      claimIds: options.claimIds || [],
    };
    store.setState(
      checked.stale ? "stale" : options.detour ? "detour" : "following",
    );
    if (stop?.base && stop?.head && changeStatus !== "A") {
      ...
      if (token !== generation) return;
      if (a.focus.some((f) => f.side === "base"))
        await waitForBase(active.baseUri);
    }
    if (token !== generation) return;
    await reconcile(true);
    if (token !== generation) return;
    const first = a.focus[0];
    if (first && !checked.stale)
      await editor.reveal(
        a.path,
        first.side === "head" && stop?.head?.sha === "WORKTREE"
          ? "working"
          : first.side,
  ...
  async function reconcile(reveal = false) {
    ...
    try {
      ...
      const fallback =
        baseFocus.length && beat.baseUri && !original && !beat.stale;
      const useCompanion =
        fallback && config().get("removedCode", "companion") === "companion";
      const companionUri = useCompanion
        ? revisionUri(beat.source, stop.base.sha, a.path, true)
        : null;
      ...
      if (useCompanion && !["exploring", "paused"].includes(store.state())) {
        ...
        if (!visible && !owned?.detached) {
          ...
          if (reveal && active === beat)
            e.revealRange(
              new vscode.Range(
                baseFocus[0].range.startLine - 1,
                0,
                baseFocus[0].range.endLine - 1,
                0,
              ),
      ...
      if (active !== beat) return;
      store.clearPaint();
      const contextSide = ["worktree", "working"].includes(a.side)
        ? "head"
        : a.side;
      ...
      narrationHover.isTrusted = {
        enabledCommands: ["kanko.presentation.openClaim"],
      };
      for (const side of ["base", "head"]) {
        ...
        const context =
          side === contextSide
            ? a.context
            : mapRange(a.context, hunks, contextSide);
        const seams =
          fallback && side === "head"
            ? baseFocus.flatMap((f) => {
                ...
                const headEditor = vscode.window.visibleTextEditors.find(
                  (e) => uriKey(e.document.uri) === uriKey(beat.headUri),
                );
                const peekLine = Math.max(
                  0,
                  Math.min(
                    line,
                    (headEditor?.document.lineCount || line + 1) - 1,
                  ),
                );
                const args = [
                  beat.headUri,
                  new vscode.Position(peekLine, 0),
                  [
                    new vscode.Location(
                      beat.baseUri,
                      new vscode.Range(
                ...
                md.appendMarkdown(
                  `[Peek removed code](command:editor.action.showReferences?${encodeURIComponent(JSON.stringify(args))})`,
                );
                md.isTrusted = {
                  enabledCommands: ["editor.action.showReferences"],
                };
                return [
                  {
                    line,
                    hover: md,
                    label: `${f.range.endLine - f.range.startLine + 1} lines removed${useCompanion ? " · shown beside" : " · peek"}`,
                  },
                ];
              })
            : [];
        store.setPaint(
          ...
          side === "head" &&
            (stop?.head?.sha === "WORKTREE" ||
              ["working", "worktree"].includes(a.side))
            ? "working"
            : side,
          {
            context: [context],
  ...
  async function focus(body, stop) {
    ...
    const context = ranges.find((r) => r.side === body.side) || {
    ...
    const anchor = {
      path: body.path,
      side,
      rev: stop?.[body.side]?.sha || "WORKTREE",
      context,
      focus: [
        {
          side: body.side === "working" ? "head" : body.side,
          range: { startLine: body.startLine, endLine: body.endLine },
        },
      ],
    };
    if (side === "worktree") {
      const doc = await vscode.workspace.openTextDocument(
        vscode.Uri.file(editor.absolute(body.path)),
      );
  ...
  const report = (err) =>
    vscode.window.showWarningMessage(`Tour presentation: ${err.message}`);
  const subscriptions = [
    provider,
    ...
    vscode.workspace.onDidChangeTextDocument((e) => {
      if (
        !active ||
        !["worktree", "working"].includes(active.anchor.side) ||
        editor.describe({ document: e.document })?.path !== active.anchor.path
      )
        return;
      const beat = active;
      clearTimeout(driftTimer);
    ...
    vscode.window.onDidChangeTextEditorSelection((e) => {
      if (e.kind && !navigating && store.state() === "following") {
        store.setState("exploring");
        repaint();
      }
    }),
  ];
  ...
  return {
    ...
    status() {
      return {
        ...
        anchor: active ? structuredClone(active.anchor) : null,
      };
    },
    async detour(anchor, stop, label) {
      saved = active;
      await activate(anchor, stop, label, { detour: true });
    },
    async returnFromDetour() {
      const prior = saved;
      saved = null;
      if (prior)
        await activate(prior.anchor, prior.stop, prior.label, {
          claimIds: prior.claimIds,
        });
    },
    dispose() {
      generation++;

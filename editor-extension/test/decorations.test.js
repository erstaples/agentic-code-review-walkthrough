  ...
  files: [
    {
      path: "a.go",
      ranges: [
        { side: "head", startLine: 12, endLine: 48 },
        { side: "base", startLine: 3, endLine: 5 },
      ],
    },
...
test("returns only the ranges matching the editor's path and side", () => {
  ...
  s.setStop(stop);
  assert.deepStrictEqual(s.rangesFor({ path: "a.go", side: "head" }).stop, [
    { startLine: 12, endLine: 48 },
  ]);
  assert.deepStrictEqual(s.rangesFor({ path: "a.go", side: "base" }).stop, [
...
test("an editor not in the stop gets nothing", () => {
  const s = createIntentStore();
  s.setStop(stop);
  assert.deepStrictEqual(s.rangesFor({ path: "z.go", side: "head" }), {
    stop: [],
    focus: null,
  });
});
...
test("focus is returned only for its own path and side", () => {
  ...
  assert.deepStrictEqual(hit.focus, {
    startLine: 31,
    endLine: 35,
    note: "capped here",
  });
...
test("a new stop replaces the previous stop and clears focus", () => {
  ...
  s.setStop({
    stopId: "s3",
    files: [
      { path: "c.go", ranges: [{ side: "head", startLine: 1, endLine: 1 }] },
    ],
  });
  assert.deepStrictEqual(s.rangesFor({ path: "a.go", side: "head" }), {
    stop: [],
    focus: null,
  });
  assert.deepStrictEqual(s.rangesFor({ path: "c.go", side: "head" }).stop, [
...
test("every (path, side) pair the stop asked for starts pending and leaves on markApplied", () => {
  const s = createIntentStore();
  s.setStop(stop);
  assert.deepStrictEqual(byKey(s.pending()), [
    "base:a.go",
    "head:a.go",
    "working:b.go",
  ]);
...
test("a file pending on both sides stays pending on the other side after one side applies", () => {
  ...
  assert.deepStrictEqual(byKey(s.pending()), ["base:a.go", "working:b.go"]);
  assert.ok(
    s.pending().some((p) => p.path === "a.go" && p.side === "base"),
    "a.go's base side must still be pending",
  );
});
...
test("setFocus without a stop still yields the focus range", () => {
  ...
  assert.deepStrictEqual(s.rangesFor({ path: "a.go", side: "working" }).focus, {
    startLine: 4,
    endLine: 4,
    note: undefined,
  });
...
test("currentFocus returns the focus fields after setFocus", () => {
  ...
  assert.deepStrictEqual(s.currentFocus(), {
    path: "a.go",
    side: "head",
    startLine: 31,
    endLine: 35,

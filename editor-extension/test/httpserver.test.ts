...
test("routes a GET to its handler and wraps the result in ok:true", async () => {
  await withServer(
    { "GET /status": async () => ({ extensionVersion: "0.1.0" }) },
    async (base) => {
...
test("a missing or wrong bearer token is unauthorized", async () => {
  await withServer({ "GET /status": async () => ({}) }, async (base) => {
    ...
    const wrong = await fetch(`${base}/status?protocolVersion=1`, {
  ...
  try {
    ...
    await Promise.race([
      ...
      new Promise((_, reject) =>
        setTimeout(() => reject(new Error("close() did not resolve")), 3000),
      ),
    ]);
  } finally {
    sock.destroy();

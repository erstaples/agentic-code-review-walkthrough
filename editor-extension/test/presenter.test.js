...
const { parseHunks, seamLineFor, removedBaseLines0, mapRange } =
...
function fixture() {
  ...
  const editor = {
    document: { lineCount: texts.length, lineAt: (i) => ({ text: texts[i] }) },
    setDecorations: (t, rs) => calls.set(t, rs),
  };
...
test("presenter preserves diff backgrounds and uses text-only dimming", () => {
  ...
  for (const [name, t] of Object.entries(p.types))
    if (name !== "companionRemoved")
      assert.equal(t.options.backgroundColor, undefined);
  assert.equal(
    p.types.companionRemoved.options.backgroundColor.id,
  ...
  assert.deepEqual(
    calls.get(p.types.dim).map((r) => r.start.line),
    [3, 4],
  );
...
test("state changes clear old paint, retain rails, and suppress stale focus", () => {
  ...
  for (const state of ["following", "exploring", "paused", "detour", "stale"]) {
    ...
    assert.equal(
      calls.get(state === "stale" ? p.types.railStale : p.types.rail).length,
      5,
    );
  }

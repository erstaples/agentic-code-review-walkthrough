...
exports.run = async () => {
  ...
  const probe = await vscode.extensions.getExtension("kanko-test.kanko-layout-spike").activate();
  const root = vscode.workspace.workspaceFolders[0].uri.fsPath;
  const real = (name) => vscode.Uri.file(path.join(root, name));
  const revision = (name, ref) => real(name).with({ scheme: "kanko-rev", query: JSON.stringify({ path: path.join(root, name), canonicalPath: path.join(root, name), ref }) });
  const base = process.env.KANKO_SPIKE_BASE, head = process.env.KANKO_SPIKE_HEAD;
  ...
  await check("language symbols on file and kanko-rev", async () => {
    ...
    assert.ok(symbols.find((s) => s.file === "service.ts" && s.scheme === "file").names.includes("kankoProbe"), "real TypeScript positive control failed");
  ...
  await check("production removed-code companion and pause", async () => {
    ...
    const text = "export function kankoProbe(value: number): number {\n  return value + 1;\n}";
    await command("kanko.presentation.activate", { anchor: { path: "service.ts", side: "base", rev: base, context: { startLine: 1, endLine: 3 }, focus: [{ side: "base", range: { startLine: 2, endLine: 2 } }], contentHash: "sha256:" + crypto.createHash("sha256").update(text).digest("hex") }, narration: "Removed line probe" });
    await delay(800);
    ...
    return { before, activated, paused, status: await command("kanko.presentation.status") };
  });

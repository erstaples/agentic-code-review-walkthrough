  ...
  act(() => {
    window.dispatchEvent(
      new window.MessageEvent("message", {
        data: { type: "snapshot", snapshot: snapshot(7, { revision: 9 }) },
      }),
    );
    window.dispatchEvent(
      new window.MessageEvent("message", {
        data: { type: "selectAnchor", anchor: 3 },
      }),
    );
  });
...
test("row controls retain their DOM identity and focus after updates", () => {
  const f = setup();
  const pin = button("Pin anchor 1");
  pin.focus();
  const next = snapshot(7, { revision: 2 });
  next.presentation.layout.slots[0].pinned = true;
  f.update(next);
  assert.equal(button("Unpin anchor 1"), pin);
...
test("paused state disables rows, picker, narration and reset actions", async () => {
  ...
  for (const name of [
    "Anchor 1: src/file-1.ts",
    "Replace 1 in top",
    "Inspect source one",
    "Reset layout",
  ])
    assert.equal(button(name).disabled, true, name);
  ...
  assert.match(
    screen.getByRole("region", { name: "Placement picker" }).textContent,
    /Open 3/,
  );
});
...
test("narration stays escaped, exposes live text, and dispatches only numbered chips", async () => {
  ...
  const chip = button("Inspect source one");
  chip.focus();
  f.update(snapshot(7, { revision: 2 }));
...
test("pin, reset, sequence override, stop navigation and end send typed requests", async () => {
  const s = snapshot();
  s.presentation.layout.sequence = true;
  const f = setup(s);
  for (const [name, expected] of [
    [
      "Pin anchor 1",
      { type: "layout", action: "pin", anchor: 1, pinned: true },
    ],
    ["Reset layout", { type: "layout", action: "reset" }],
  ...
  f.update({ loaded: false, revision: 2 });
  assert.ok(screen.getByText("Load a tour from your agent to begin."));
});
...
test("unmount removes message and keyboard listeners and disconnects observers", () => {
  ...
  fireEvent.keyDown(document, { key: "0", code: "Digit0", altKey: true });
  assert.equal(f.bridge.getState(), state);
  assert.deepEqual(f.messages, [{ type: "ready" }]);
});
...
test("a removed placement choice returns focus to an available choice", async () => {
  const f = setup();
  await f.user.click(button("Open anchor 3: file-3.ts"));
  assert.equal(document.activeElement, button("Replace 1 in top"));
  const next = snapshot(7, { revision: 2 });

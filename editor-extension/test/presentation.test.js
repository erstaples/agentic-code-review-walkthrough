...
function fixture({ sideBySide = false, settings = {} } = {}) {
  class Uri {
    constructor(fsPath, scheme = "file", query = "") {
      Object.assign(this, { fsPath, scheme, query });
    }
    with(change) {
      return Object.assign(
        new Uri(this.fsPath, this.scheme, this.query),
        change,
      );
    }
    toString() {
  ...
  const event = () => {
    ...
    return {
      listen(fn) {
        handlers.add(fn);
        return { dispose: () => handlers.delete(fn) };
      },
      fire(value) {
  ...
  const makeEditor = (uri, viewColumn = 1) => ({
  ...
  const vscode = {
    ...
    Range: class {
      constructor(start, c, end, ec) {
        this.start = { line: start, character: c };
        this.end = { line: end, character: ec };
      }
    },
    Location: class {
    ...
    workspace: {
      getConfiguration: (section) => ({
        get: (key, fallback) =>
          section === "diffEditor" && key === "useInlineViewWhenSpaceIsLimited"
            ? !sideBySide
      ...
      onDidChangeConfiguration: config.listen,
      onDidChangeTextDocument: edits.listen,
    },
    window: {
      visibleTextEditors: [],
      onDidChangeVisibleTextEditors: visible.listen,
      onDidChangeTextEditorSelection: selection.listen,
      showWarningMessage: (message) => {
        throw new Error(message);
      },
      tabGroups: {
        all: [{ viewColumn: 2, tabs: [] }],
        async close(tab) {
          closed.push(tab);
          this.all[0].tabs = this.all[0].tabs.filter((t) => t !== tab);
          vscode.window.visibleTextEditors =
            vscode.window.visibleTextEditors.filter(
              (e) => e.document.uri.toString() !== tab.input.uri.toString(),
            );
        },
      },
      async showTextDocument(doc, options) {
        ...
        this.tabGroups.all[0].tabs.push({
          input: { uri: doc.uri },
    ...
    commands: {
      async executeCommand(name, base, head) {
        calls.push(name);
        vscode.window.visibleTextEditors = [
          makeEditor(head),
          ...(sideBySide ? [makeEditor(base)] : []),
        ];
      },
    },
  };
  ...
  const git = {
    changedFiles: async () => [
      { targetPath: "a.js", sourcePath: "old.js", status: "R" },
    ],
    diffHunks: async () => [
      { baseStart: 2, baseLen: 1, headStart: 1, headLen: 0 },
    ],
    blobText: async (_, ref) =>
      ref === "base" ? "a\nremoved\nlast" : "a\nlast",
  };
  ...
  const anchor = {
    ...
    focus: [
      { side: "base", range: { startLine: 2, endLine: 2 } },
      { side: "head", range: { startLine: 2, endLine: 2 } },
    ],
  };
  return {
    presentation,
...
test("inline focus opens a diff plus a marked companion with mapped rails and removed backgrounds", async (t) => {
  ...
  assert.deepEqual(f.store.paintFor("a.js", "base").context, [
...
test("side-by-side layout paints both sides without opening a companion", async (t) => {
  const f = fixture({ sideBySide: true });
  t.after(() => f.presentation.dispose());
  ...
  assert.match(
    f.store.paintFor("a.js", "base").hover.value,
    /command:kanko.presentation.openClaim/,
  );
  assert.deepEqual(
    f.store.paintFor("a.js", "base").hover.isTrusted.enabledCommands,
    ["kanko.presentation.openClaim"],
  );
});

test("seam preference uses an allowlisted peek link and no companion", async (t) => {
  const f = fixture({ settings: { removedCode: "seam" } });
  t.after(() => f.presentation.dispose());
  await f.presentation.activate(f.anchor, f.stop, "claim");
  assert.equal(f.calls.filter((c) => c.show).length, 0);
  const seam = f.store.paintFor("a.js", "head").seams[0];
  assert.match(seam.hover.value, /command:editor.action.showReferences/);
  assert.deepEqual(seam.hover.isTrusted.enabledCommands, [
...
test("pinned or moved companions are not closed and lifetime preferences are respected", async (t) => {
  for (const mode of ["pinned", "moved", "never", "onTourEnd"]) {
    const settings = {
      closeCompanion: ["never", "onTourEnd"].includes(mode)
        ? mode
        : "onBeatChange",
    };
    ...
    await f.presentation.activate(f.anchor, f.stop, "claim");
    if (mode === "pinned")
      f.vscode.window.tabGroups.all[0].tabs[0].isPreview = false;
...
test("detour replaces the beat, Return restores it, and stale context suppresses fallback", async (t) => {
  ...
  await f.presentation.activate(
    { ...f.anchor, contentHash: hashText("wrong") },
    f.stop,
    "stale",
  );
...
test("worktree edits are rehashed after debounce, even when line counts stay the same", async (t) => {
  ...
  const a = {
    ...
    context: { startLine: 1, endLine: 2 },
    contentHash: hashText("a\nlast"),
    focus: [{ side: "head", range: { startLine: 2, endLine: 2 } }],
  };

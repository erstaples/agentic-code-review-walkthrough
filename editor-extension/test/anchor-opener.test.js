...
const { createAnchorOpener } = require("./compiled.js")(
  "src/host/anchor-opener.js",
);
const { sourceHunks } = require("./compiled.js")("src/host/source-diff.js");
const { createDecorationRegistry } = require("./compiled.js")(
  "src/host/decoration-registry.js",
);
function fixture(t) {
  ...
  const groups = [{ viewColumn: 1, tabs: [], activeTab: null }],
    textDocuments = [],
    closed = [],
    handlers = [];
  const event = (fn) => {
  ...
  const vscode = {
    Uri: {
      file: (p) => uri("file", p),
      from: (o) => uri(o.scheme, o.path, o.query),
    },
    ViewColumn: { Beside: -2 },
    workspace: {
      textDocuments,
    ...
    window: {
      ...
      showTextDocument: async (doc, options) => show({ uri: doc.uri }, options),
    },
    commands: {
      executeCommand: async (_, original, modified, label, options) =>
  ...
  function show(input, options) {
    ...
    let tab = g.tabs.find(
      (t) =>
        (t.input.uri || t.input.modified).toString() ===
        (input.uri || input.modified).toString(),
    );
    if (!tab) {
      const old = g.tabs.find((t) => t.isPreview);
      if (old) g.tabs = g.tabs.filter((t) => t !== old);
      tab = { input, isPreview: true, isPinned: false, isDirty: false };
      g.tabs.push(tab);
    }
    g.activeTab = tab;
    emit();
  ...
  const opener = createAnchorOpener(vscode);
...
test("existing reviewer tabs are reused in their group and never adopted or closed", async (t) => {
  ...
  const first = await f.opener.open(r, 1, { focus: true });
  const second = await f.opener.open(r, 1);
  assert.equal(first.tab, user);
  assert.equal(second.tab, user);
  assert.equal(f.groups.length, 1);
  assert.equal(f.opener.disposable(user), false);
...
test("opener rejects a protected preview and an unallocated group", async (t) => {
  ...
  assert.ok(f.groups[0].tabs.includes(user));
  const next = {
    ...r,
...
test("the registry reuses six palette sets, keeps distinct colors, and disposes them", () => {
  ...
  assert.equal(first.types.rail.options.borderColor.id, "kanko.anchor1");
  assert.equal(
    registry.forAnchor(2).types.boxOne.options.borderColor.id,
    "kanko.anchor2",
  );
...
test("a removed-code companion uses a new third column, never the occupied adjacent one", async (t) => {
  ...
  assert.ok(
    f.groups[1].tabs.some(
      (t) => t.input.uri.toString() === otherUri.toString(),
    ),
  );
});

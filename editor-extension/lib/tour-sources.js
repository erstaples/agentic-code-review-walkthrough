...
function tourSources(workspace, change) {
  ...
  const readWorking = (file) => {
    if (file.untracked || file.unstaged) {
      ...
      invariant(digest(bytes) === file.working.digest, "stale_change", "working source changed; refresh the record");
      return bytes;
    }
    if (file.staged) {
  ...
  return {
    ...
    readSource(anchor) {
      ...
      invariant(anchor.rev.base === revisions.base && anchor.rev.head === revisions.head, "revision_mismatch", "anchor revisions do not match this record");

...
before proposing durable dossier content. Resolve the absolute Git repository
root and inspect the current status so pre-existing user changes remain visible
and untouched.

Open a working-tree dossier with `dossier_open`:
...
- Give it a short task-oriented title and identify the coding agent as the
  actor.
- Reuse a matching dossier only when its title, thesis, and requirements belong
  to this task. If an exact or related dossier belongs to another task, call
  `dossier_open` again with `forceNew: true`; never merge unrelated task history
  for convenience.
- Keep the returned dossier ID and latest aggregate revision. Every mutation
...
If dossier tools are unavailable, continue the implementation and state at
handoff that durable capture was unavailable. Do not block ordinary coding on
the editor extension; dossier tools do not require it.

## Seed durable context

Record the useful source material early through typed `dossier_apply` commands:
...
Update the dossier at meaningful decision points, not after every command or
edit:
...
1. Call `dossier_refresh` with the final working-tree selection. Treat earlier
...
logical, commit-message-worthy unit and cover at least one dossier entity unless
...
Then run `dossier_check`. If the candidate is stale, refresh and repair the
...
belong to the later `tour-changes` walkthrough.

In the implementation handoff, report the dossier ID, final change identity,
